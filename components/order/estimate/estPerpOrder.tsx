...
import PriceEntrySummary from "../common/entryPriceDisplay"
// ui
              ...
              <div className="border-b dark:border-gray-700">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10 shadow-sm">
                    <tr>
                      {["Grid", "Entry Condition", "Order Size", "Leverage", "Exit / TP / SL", "Wallet", "Status", "Actions"].map(
                        (head) => (
                          <th
                            key={head}
                    ...
                    {estOrders.map((order, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            {hasTechEntry ? (
                              ...
                              isEditing ? (
                                <div className="flex items-center gap-1">
                                  <PriceEntrySummary
                                    priceEntry={currentOrder.entry.priceEntry!}
                                  />
                                  {/* <input
                                    type="number"
                                    className="w-24 px-2 py-1 text-sm border rounded dark:bg-gray-700 dark:border-gray-600"
                                    value={currentOrder.entry.priceEntry!.targetPriceUsd}
                                    onChange={(e) =>
                                      handleFieldChange(orderId, "entry.priceEntry.targetPriceUsd", e.target.value)
                                    }
                                  /> */}
                              ...
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  value={formateAmountWithFixedDecimals(
                                    currentOrder.perp?.amount?.orderSize ?? "0",
                                    collateralToken?.decimals ?? 18,
                                    3
                                  )}
                                  onChange={(e) =>
                                    handleFieldChange(orderId, "perp.amount.orderSize", e.target.value)
                                  }
                                />
                          ...
                          {/* Leverage */}
                          <td
                            className={`px-4 py-3 text-sm text-[10px] font-bold ${currentOrder.perp?.leverage && currentOrder.perp?.leverage > 10
                              ? "text-orange-500"
                              : "text-green-500"
                              } uppercase flex items-center gap-1`}
                          >
                            {currentOrder.perp?.leverage}X
                          </td>
                  ...
                  >
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-900/50 p-2 rounded">
                        ...
                        {currentOrder.entry.technicalLogic ? (
                          ...
                          <div className="font-mono">
                            <PriceEntrySummary priceEntry={currentOrder.entry.priceEntry} />
                          </div>
                        ) : (
                          <span className="text-gray-400 italic">Market</span>
                        )}
                      ...
                      <div className="flex justify-between text-xs text-gray-600 dark:text-gray-400">
                        ...
                        <span>Leverage: {currentOrder.perp?.leverage}X</span>
                      </div>
                    </div>
                  </div>
                );
